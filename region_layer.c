...
int region_layer_init(region_layer_t *rl, int width, int height, int channels, int origin_width, int origin_height)
{
    ...
    rl->boxes_number = (rl->layer_width * rl->layer_height * rl->anchor_number); // 20*14*10=2800
    rl->output_number = (rl->boxes_number * (rl->classes + rl->coords + 1)); // 2800 * (1+4+1) = 16800
